...
public class KFMP4Demuxer {
    ...
    public KFMP4Demuxer(KFDemuxerConfig config, KFDemuxerListener listener) {
        ...
        ///< 初始化音频解封装器
        Log.d(TAG, "KFMP4Demuxer: hasAudio():"+hasAudio()+","+(config.demuxerType.value() & KFMediaBase.KFMediaType.KFMediaAudio.value()));
    ...
    public boolean hasVideo() {
        ...
        if(mConfig.path.equals("")){
           ...
           value  = mRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_HAS_VIDEO);
        }
    ...
    public ByteBuffer readAudioSampleData(MediaCodec.BufferInfo bufferInfo){
        ///< 音频数据读取
        if (mAudioMediaExtractor == null) {
            Log.d(TAG, "mAudioMediaExtractor == null: ");
            return null;
        }
            ...
            try {
                if(mConfig.path.equals("")){
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        Log.d(TAG, "mConfig.path is : ");
                        mAudioMediaExtractor.setDataSource(mConfig.afd);
                    }
                }else{
                    Log.d(TAG, "mConfig.path is not null: ");
